...
func (h *Handler) TelemetryMQTT(w http.ResponseWriter, r *http.Request) {
	const op = "Handler.TelemetryMQTT"
	ctx := r.Context()
	log := h.log.With("op", op)
	log.Info("request started", "method", r.Method, "url", r.URL.String())

	var body telemetryMQTTBody
	...
	req := h.ReqConv.ToTelemetryMQTT(body)
	result, err := h.ReqConv.IngestTelemetryMQTT(ctx, h.telemetryService, req)
	if err != nil {
		handleError(ctx, w, err, log, "ingest mqtt telemetry failed")
		return
	}
